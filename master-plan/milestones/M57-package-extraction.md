...
8. Update callers in root package:
   - `config.go`: `scanYAML(...)` → `yamlmini.ScanYAML(...)`, `parseNestedMap(...)` → `yamlmini.ParseNestedMap(...)`, `parseFlatMap(...)` → `yamlmini.ParseFlatMap(...)`. Add import `"codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini"`
...
- `config.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini"`
- `vault.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini"`
...
6. Add import to `internal/config/config.go`: `"codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini"` (for `ScanYAML`, `ParseNestedMap`, `ParseFlatMap`)
...
**Import budget:**
- `internal/config/config.go`: `os`, `path/filepath`, `strings`, `time`, `github.com/charmbracelet/lipgloss`, `codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini`
- `main.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/config"`
- `model.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/config"`
- `profile_handler.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/config"`
- `theme.go`: add `"codeberg.org/lthiagol/obsidian-terminal/internal/config"`
...
8. Add import to `internal/vault/vault.go`: `"codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini"` (for `ScanYAML` in `ParseFrontmatter`)
...
**Import budget:**
- `internal/vault/vault.go`: `os`, `path/filepath`, `regexp`, `sort`, `strings`, `codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini`
- `internal/vault/wikilink.go`: `strings`, `path/filepath` (already imported)
- All 9 caller files in root: add `"codeberg.org/lthiagol/obsidian-terminal/internal/vault"`
...
- `model.go` (or new `session_handler.go`): add `"codeberg.org/lthiagol/obsidian-terminal/internal/session"`, `"codeberg.org/lthiagol/obsidian-terminal/internal/vault"` (for `VaultEntry` in `collectExpandedDirs` — but `FileTree` stores `VaultEntry` internally, so the helper reads `item.entry.Path` which is a string; the `vault` import may not be needed if `FileTree` exposes the path as a string)
...
1. Verify package structure: `go list ./...` shows all packages:
   - `codeberg.org/lthiagol/obsidian-terminal` (main)
   - `codeberg.org/lthiagol/obsidian-terminal/internal/ansiext`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/config`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/markdown`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/search`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/session`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/vault`
   - `codeberg.org/lthiagol/obsidian-terminal/internal/yamlmini`
...
3. Verify no `main` import from `internal` packages (main imports internal, not vice versa):
   ```bash
   rg 'codeberg.org/lthiagol/obsidian-terminal"' internal/  # should return 0 matches
   ```
...
- [ ] `rg 'codeberg.org/lthiagol/obsidian-terminal"' internal/` returns 0 matches (no internal package imports main)
- [ ] `make test && make vet` pass
- [ ] `STATUS.md` M57 → ✅
